<form method="POST" action="{{ ($model != "") ? Admin::route('contentManager.post.update',['post'=>$model->id]) : Admin::route('contentManager.post.store') }}">
  <div class="col-md-9">
    ...
    <div class="form-group">
        ...
        <p class="help-block"><strong>Permalink: </strong>
          <span id="slug-permalink" title="Click to preview">
            <a href="{{ Url('/') }}/{{ $model->post_name }}" target="_blank">{{ Url('/') }}/{{ $model->post_name }}</a>
          </span>
        </p>
        @endif
    </div>
    <div class="form-group">
        ...
        <textarea id="content-post" name="post_content" class="form-control ckeditor" rows="18">{{ ($model != "" ) ? Helper::bbcode($model->post_content) : old('post_content') }}</textarea>
    </div>
    <div class="form-group">
